...
class FXAAShader extends h3d.shader.ScreenShader {
	static var SRC = {
		...
		@param var spanMul:Float;
		@param var reduceMinInv:Float;
		@param var blendStrength:Float;
		...
		function fragment() {
			...
			var dirReduce = max((lumaNW + lumaNE + lumaSW + lumaSE) * 0.25 * 0.5, 1.0 / reduceMinInv);
			var rcpDirMin = 1.0 / (min(abs(dir.x), abs(dir.y)) + dirReduce);

			dir = clamp(dir * rcpDirMin * spanMul, vec2(-8.0, -8.0), vec2(8.0, 8.0)) * rcpFrame;
...
class FXAAFilter extends Shader<FXAAShader> {
	public var spanMul:Float = 1.0 / 8.0;
	public var reduceMinInv:Float = 128.0;
	public var blendStrength:Float = 1.0;
