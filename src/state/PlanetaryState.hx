...
class PlanetaryState extends HState {
	...
	function setup():Void {
		...
		rotateButton.onClick((_) -> {
			if (mode == WaitingEffect || mode == CameraPanning || mode == MenusOpen)
				return;
			mode = WaitingEffect;
	...
	function openActionMenu():Void {
		if (menuCircler != null || mode == MenusOpen)
			return;
...
class PlanetaryStateControls {
	...
	public function readMovement():{x:Float, y:Float, anyPress:Bool} {
		...
		var moveY = 0.0;
		var anyPress = false;
		if (controls.isPressed(MoveUp)) {
			moveY += 1;
			anyPress = true;
		}
		if (controls.isPressed(MoveDown)) {
			moveY -= 1;
			anyPress = true;
		}
