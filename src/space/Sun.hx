...
class Sun extends h3d.scene.Object {
    ...
    public function new(parent: h3d.scene.Object) {
        ...
        spherePrim.addNormals();         spherePrim.addUVs();         var sunMesh = new Mesh(spherePrim, this);          sunMesh.scale(3);  
        sunMesh.material = h3d.mat.Material.create();
        sunMesh.material.mainPass.enableLights = false;          sunMesh.material.color.set(50, 50, 37.5);  
                var sunDir = sunPos.clone();
        sunDir.normalize();
        sunDir.scale(-1);
        var sunLight = new DirLight(sunDir, parent);          sunLight.power = 5;          sunLight.color.set(1, 1, 0.9);
