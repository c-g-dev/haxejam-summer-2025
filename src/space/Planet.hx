...
class Planet extends h3d.scene.Object {
	...
	public function new(parent:Object, s2d:h2d.Scene, camera:h3d.Camera) {
		...
		var tex = hxd.Res.topography.toTexture();
		tex.wrap = Clamp;
		tex.filter = Linear;
		sphereMesh.material.texture = tex;

		sphere.addChild(sphereMesh);
	...
	private function initGrid(s2d:h2d.Scene) {
		...
		newVerts = verts.copy();
		var midMap = new Map<String, Int>();
		subFaces = [];
		function getMid(a:Int, b:Int):Int {
			var min = a < b ? a : b;
			var max = a > b ? a : b;
			var key = '$min-$max';
			if (midMap.exists(key))
		...
		for (f in faces) {
			...
			var e = getMid(b, c);
			var mf = getMid(c, a);
			subFaces.push([a, d, mf]);
			subFaces.push([b, e, d]);
			subFaces.push([c, mf, e]);
			subFaces.push([d, e, mf]);
		}
		...
		function drawArc(v1:h3d.Vector, v2:h3d.Vector, segments:Int = 16) {
			...
			for (i in 0...segments) {
				...
				var p1 = new h3d.Vector(a1 * v1.x + a2 * v2.x, a1 * v1.y + a2 * v2.y, a1 * v1.z + a2 * v2.z);
				var p2 = new h3d.Vector(b1 * v1.x + b2 * v2.x, b1 * v1.y + b2 * v2.y, b1 * v1.z + b2 * v2.z);
				p1.normalize();
				p2.normalize();
	...
	function buildTriangleOverlays():Void {
		...
		for (i in 0...subFaces.length) {
			var sf = subFaces[i];
			var v1 = newVerts[sf[0]].clone();
			v1.scale(pushOut);
			var v2 = newVerts[sf[1]].clone();
			v2.scale(pushOut);
			var v3 = newVerts[sf[2]].clone();
			v3.scale(pushOut);
	...
	public function colorTriOverlay(index:Int, r:Float, g:Float, b:Float, a:Float):Void {
		if (index < 0 || index >= triColors.length)
			return;
